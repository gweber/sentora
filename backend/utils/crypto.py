...
def decrypt_field(stored: str | None) -> str | None:
    ...
    If the value is ``None`` or empty, it is returned as-is so that
    callers never crash on missing MongoDB fields (AUDIT-032).

    If the value is not ``enc:``-prefixed (legacy plaintext), it is
    returned as-is for backwards compatibility.

    Args:
        stored: The stored value (encrypted, plaintext, or ``None``).

    Returns:
        The decrypted plaintext string, or ``None`` if the input was ``None``.
    """
    if not stored:
        return stored
