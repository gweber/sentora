...
async def ensure_all_indexes(db: AsyncIOMotorDatabase) -> None:  # type: ignore[type-arg]
    ...
    # TTL index — auto-expire pending OIDC states after 10 minutes
    ...
    # TTL index — auto-expire pending SAML request IDs after 10 minutes
    await db["saml_pending_requests"].create_index(
        "created_at", expireAfterSeconds=600, background=True
    )

    # ── saml_token_exchange (one-time SAML nonce → JWT token pairs) ───────
    # TTL index — auto-expire exchange nonces after 5 minutes
