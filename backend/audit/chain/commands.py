...
async def initialize_chain(
    ...
    *,
    ...
    # Check if genesis already exists (fast path)
    existing = await db["audit_log"].find_one({"sequence": 0})
    if existing:
        # Sync sequence counter to actual max in DB — prevents duplicate
        # sequence allocation after process restart (AUDIT-023)
        max_doc = await db["audit_log"].find_one(
            {"sequence": {"$exists": True}},
            sort=[("sequence", -1)],
            projection={"sequence": 1},
        )
        if max_doc is not None:
            await reset_sequence_to(db, max_doc["sequence"] + 1)
        return {k: v for k, v in existing.items() if k != "_id"}

    # No genesis — allocate sequence and create
    ...
    # Atomic upsert: use $setOnInsert so only the first writer creates the
    # genesis entry.  This eliminates the check-then-insert race where two
    # concurrent callers both see no genesis and both attempt to insert.
    result = await db["audit_log"].update_one(
        {"sequence": 0},
        {"$setOnInsert": entry},
        upsert=True,
    )
    ...
    # Ensure counter is at 1 so next allocation returns 1 (after genesis at 0)
    await reset_sequence_to(db, 1)

    # Return the canonical genesis entry from the database
    existing = await db["audit_log"].find_one({"sequence": 0})
    if existing:
...
async def append_chained_entry(
    ...
    *,
    ...
    """
    # Use a distributed lock to serialize the entire allocate-hash-insert
    # sequence.  Without this, two concurrent callers could both allocate
    # adjacent sequences but the second caller's ``get_previous_hash`` could
    # execute before the first caller's insert completes, returning a stale
    # hash and breaking the chain (AUDIT-007).
    import asyncio

    lock = DistributedLock(db, "audit_chain_append", ttl_seconds=30)
    _MAX_LOCK_ATTEMPTS = 10
    for _lock_attempt in range(_MAX_LOCK_ATTEMPTS):
        acquired = await lock.acquire()
        if acquired:
            break
        await asyncio.sleep(0.05 * (2**_lock_attempt))
    else:
        from errors import ChainIntegrityError

        raise ChainIntegrityError(
            "Could not acquire chain append lock after retries",
            detail={"lock": "audit_chain_append"},
        )
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 0:
                    ...
                    entry.pop("_id", None)
                    continue
                # Second attempt failed — decrement sequence counter to avoid
                # a permanent gap in the chain (AUDIT-022)
                from errors import ChainIntegrityError

                logger.error(
                    "Chain entry write permanently failed (seq={}): {} — "
                    "rolling back sequence counter to prevent gap",
                    sequence,
                    exc,
                )
                await reset_sequence_to(db, sequence)
                raise ChainIntegrityError(
