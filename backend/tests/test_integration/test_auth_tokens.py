...
class TestAdminUserManagement:
    ...
    async def test_disable_user(self, client: AsyncClient, admin_headers: dict) -> None:
    ...
    async def test_enable_user(self, client: AsyncClient, admin_headers: dict) -> None:
...
class TestVerifyTokenEdgeCases:
    ...
    def test_refresh_token_rejected_as_access(self) -> None:
        ...
        token = pyjwt.encode(
            {"sub": "user", "role": "viewer", "type": "refresh", "exp": 9999999999},
    ...
    def test_token_missing_sub_rejected(self) -> None:
        ...
        settings = get_settings()
        token = pyjwt.encode(
            {"role": "viewer", "type": "access", "exp": 9999999999},
            settings.jwt_secret_key,
            algorithm=settings.jwt_algorithm,
        )
    ...
    def test_token_missing_role_rejected(self) -> None:
        ...
        settings = get_settings()
        token = pyjwt.encode(
            {"sub": "user", "type": "access", "exp": 9999999999},
            settings.jwt_secret_key,
            algorithm=settings.jwt_algorithm,
        )
        with pytest.raises(ValueError, match="missing required"):
