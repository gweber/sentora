...
# Entries are evicted after ``_LIMITER_TTL_SECONDS`` of inactivity to prevent
# unbounded memory growth when many distinct API keys are used over time.
_minute_limiters: dict[str, tuple[RateLimiter, float]] = {}
_hour_limiters: dict[str, tuple[RateLimiter, float]] = {}

#: Evict limiter entries that have not been accessed for this many seconds.
_LIMITER_TTL_SECONDS: float = 3600.0  # 1 hour

#: How often (in number of get calls) to run the eviction sweep.
_LIMITER_CLEANUP_INTERVAL: int = 100
_limiter_access_count: int = 0


def _evict_stale_limiters() -> None:
    """Remove limiter entries not accessed within the TTL window."""
    import time

    cutoff = time.monotonic() - _LIMITER_TTL_SECONDS
    for store in (_minute_limiters, _hour_limiters):
        stale_keys = [k for k, (_, ts) in store.items() if ts < cutoff]
        for k in stale_keys:
            del store[k]
...
def _get_minute_limiter(key_id: str, limit: int) -> RateLimiter:
    ...
    global _limiter_access_count
    _limiter_access_count += 1
    if _limiter_access_count % _LIMITER_CLEANUP_INTERVAL == 0:
        _evict_stale_limiters()

    now = time.monotonic()
    entry = _minute_limiters.get(key_id)
    if entry is None or entry[0]._max != limit:
        limiter = RateLimiter(max_requests=limit, window_seconds=60)
        _minute_limiters[key_id] = (limiter, now)
        return limiter
    _minute_limiters[key_id] = (entry[0], now)
    return entry[0]
...
def _get_hour_limiter(key_id: str, limit: int) -> RateLimiter:
    ...
    now = time.monotonic()
    entry = _hour_limiters.get(key_id)
    if entry is None or entry[0]._max != limit:
        limiter = RateLimiter(max_requests=limit, window_seconds=3600)
        _hour_limiters[key_id] = (limiter, now)
        return limiter
    _hour_limiters[key_id] = (entry[0], now)
    return entry[0]
...
async def get_auth_context(
    ...
    # TenantMiddleware sets ``request.state.tenant`` (a dict with a "slug"
    # key) and ``request.state.tenant_db_name``.  Fall back to "default"
    # only for single-tenant / on-prem deployments where no middleware runs.
    _state = getattr(request, "state", None)
    tenant_doc = getattr(_state, "tenant", None)
    if isinstance(tenant_doc, dict) and tenant_doc.get("slug"):
        tenant_id = tenant_doc["slug"]
    else:
        tenant_id = getattr(_state, "tenant_db_name", "default")
    return AuthContext(
        auth_type="jwt",
        tenant_id=tenant_id,
        user_id=payload.sub,
