...
#: Maximum number of violations fetched from each source when building the
#: unified violations view.  Prevents unbounded memory usage while still
#: allowing deep pagination up to this limit.
_MAX_UNIFIED_FETCH = 1000
...
async def list_unified_violations(
    ...
    To produce correct pagination across two independent sources, this
    endpoint fetches all violations up to ``page * page_size`` from each
    source (capped at ``_MAX_UNIFIED_FETCH`` to bound memory), merges them
    globally by severity priority then date descending, and slices the
    requested page from the merged result.
    """
    all_violations: list[UnifiedViolationResponse] = []
    total = 0

    # We need all items up to the end of the requested page so we can
    # globally sort before slicing. Cap the fetch to avoid memory issues.
    fetch_limit = min(page * page_size, _MAX_UNIFIED_FETCH)

    # Compliance violations — fetch first ``fetch_limit`` items (page 1)
    if source in (None, "compliance"):
        compliance_result = await queries.list_violations(
            db, None, severity, page=1, page_size=fetch_limit
        )
    ...
    # Enforcement violations — fetch first ``fetch_limit`` items (page 1)
    if source in (None, "enforcement"):
        ...
        enforcement_result = await enforcement_service.list_current_violations(
            db, severity, page=1, page_size=fetch_limit
        )
    ...
    # Sort merged results by severity priority then checked_at descending
    severity_order = {"critical": 0, "high": 1, "medium": 2, "low": 3}
    all_violations.sort(
        key=lambda v: (severity_order.get(v.severity, 9), v.checked_at),
    )

    # Slice the globally-sorted list to the requested page
    skip = (page - 1) * page_size
    paginated = all_violations[skip : skip + page_size]
