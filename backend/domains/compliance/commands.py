...
async def _dispatch_compliance_webhooks(
    ...
    *,
    ...
    try:
        ...
        logger.opt(exception=True).warning("Failed to dispatch compliance webhooks")
...
async def update_schedule(
    ...
    *,
    ...
    """
    # Validate cron expression before persisting (AUDIT-048).
    if cron_expression is not None:
        import re

        cron_re = re.compile(r"^(\*|[0-9,\-\/]+)(\s+(\*|[0-9,\-\/]+)){4}$")
        if not cron_re.match(cron_expression.strip()):
