...
class ClassificationManager:
    ...
    async def trigger(
        ...
        Uses a boolean flag checked before creating the background task to
        prevent the race window between ``create_task()`` and the point where
        the background task acquires ``self._lock`` (AUDIT-010).
        ...
        """
        # Fast local check — prevents a second call from slipping through
        # before the background task acquires self._lock (AUDIT-010).
        if self._running:
            logger.warning("Classification run already in progress (local flag)")
            return None
        ...
        # Set the flag atomically (single-threaded event loop) before creating
        # the task so no second call can slip through between here and the
        # _lock.acquire() inside _run_classification.
        self._running = True
        ...
        async with self._lock:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "Classification run {}: config load failed, using hardcoded "
                        "thresholds (score_clear={}, score_partial={}, gap_clear={}): {}",
                        run_id,
                        _SCORE_CLEAR,
                        _SCORE_PARTIAL,
                        _GAP_CLEAR,
                ...
                # ── Process agents in batches (keyset pagination) ────────────
                # Uses keyset pagination (sort by _id, filter with $gt) instead
                # of skip+limit to avoid skipping or double-classifying agents
                # when documents are inserted/deleted between batches (AUDIT-034).
                from bson import ObjectId as _ObjectId

                last_seen_id: _ObjectId | None = None
                processed = 0

                while True:
                    batch_filter: dict[str, Any] = {}
                    if last_seen_id is not None:
                        batch_filter["_id"] = {"$gt": last_seen_id}
                    ...
                    try:
                        ...
                        msg = f"Failed to fetch agent batch (after_id={last_seen_id}): {exc}"
                    ...
                    if batch_results:
                        try:
                            ...
                            if failed > 0:
                                msg = (
                                    f"Batch after_id={last_seen_id}: "
                                    f"{failed}/{len(batch_results)} "
                            ...
                            msg = f"Bulk upsert failed for batch after_id={last_seen_id}: {exc}"
                            logger.error("Classification run {}: {}", run_id, msg)
                            error_log.append(msg)
