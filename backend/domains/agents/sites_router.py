"""Sites router — read-only view of synced site data."""
...
async def list_sites(
    ...
    """List synced sites with pagination and group/agent counts.
    ...
    skip = (page - 1) * limit
    sites_cursor = db[SITES].find({}, {"_id": 0}).sort("name", 1).skip(skip).limit(limit)
    sites = [doc async for doc in sites_cursor]

    site_ids = [s["source_id"] for s in sites]
    ...
    group_counts = {
        doc["_id"]: doc["group_count"] async for doc in db[GROUPS].aggregate(group_pipeline)
    }
    ...
    agent_counts = {
        doc["_id"]: doc["agent_count"] async for doc in db[AGENTS].aggregate(agent_pipeline)
    }
