...
async def update_category(
    ...
    # Rename entries first, then update the category document. If the
    # second write fails, roll back the entry rename so entries are not
    # orphaned under the new key. True atomicity would require a MongoDB
    # transaction (replica-set only) (AUDIT-038).
    ...
    try:
        ...
        # Roll back the entry rename to avoid orphaned entries.
        logger.warning(
            "Category doc update failed for '{}'; rolling back entry rename.", category_key
        )
        rollback_key = category_key if request.key is not None else None
        rollback_display = (
            (cat_doc.get("display") or category_key) if request.display is not None else None
        )
        new_key_for_rollback = request.key if request.key is not None else category_key
        await repository.update_category(
            db,
            new_key_for_rollback,
            new_key=rollback_key,
            new_display=rollback_display,
        )
        raise
