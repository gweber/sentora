...
async def create_fingerprint(
    ...
    Uses the unique index on ``group_id`` (defined in ``db_indexes.py``) as
    the authoritative guard against duplicates, catching ``DuplicateKeyError``
    from the insert to close the TOCTOU race between the existence check and
    the write (AUDIT-039).
    ...
    Raises:
        FingerprintAlreadyExistsError: If a fingerprint already exists for this group.
    """
    ...
    try:
        ...
        raise FingerprintAlreadyExistsError(
            f"A fingerprint already exists for group '{group_id}'"
        ) from exc
    await audit(
        db,
        domain="fingerprint",
