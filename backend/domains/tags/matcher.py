...
async def find_matching_agents(
    ...
    if exact_patterns and not glob_patterns:
        # All patterns are exact — let MongoDB do all the filtering
        query["installed_app_names"] = {"$in": exact_patterns}
    elif exact_patterns:
        # Mix of exact and glob — use exact patterns as a pre-filter to narrow
        # the candidate set, then apply glob patterns in Python on the results
        query["installed_app_names"] = {"$in": exact_patterns}
    ...
    async for doc in cursor:
        ...
        agent_app_set: set[str] = set(agent_apps)
        matched_patterns: list[str] = []

        if exact_patterns and not glob_patterns:
            # All patterns are exact and MongoDB already filtered
            for ep in exact_patterns:
                if ep in agent_app_set:
                    matched_patterns.append(ep)
        else:
            for p in rule.patterns:
                if _is_exact_pattern(p.pattern):
                    if p.pattern in agent_app_set:
