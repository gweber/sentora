...
class SourceRunner:
    ...
    async def _execute(
        ...
        *,
        ...
        # Only applies to the nist_cpe source — other sources don't use
        # CPE-format upstream_ids and should not have entries deleted.
        if not is_resume and self.source_name == "nist_cpe":
            ...
            async for raw_entry in self._adapter.fetch(config, resume_state=resume_state):
                ...
                try:
                    ...
                    existing = await db["library_entries"].find_one(
                        {
                            "name": {"$regex": f"^{re.escape(raw_entry.name)}$", "$options": "i"},
    ...
    # Keys that must never be persisted to checkpoint documents.
    _SENSITIVE_CONFIG_KEYS = frozenset({"api_key", "secret", "token", "password"})
    ...
    async def _save_progress_checkpoint(
        ...
        Sensitive keys (e.g. ``api_key``) are stripped from the persisted
        config to avoid storing secrets in plaintext checkpoint documents.
        ...
        safe_config = {k: v for k, v in config.items() if k not in self._SENSITIVE_CONFIG_KEYS}
