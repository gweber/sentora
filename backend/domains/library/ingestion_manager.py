...
class IngestionManager:
    ...
    async def trigger_all(
        ...
        config: dict[str, Any] | None = None,
        per_source_config: dict[str, dict[str, Any]] | None = None,
    ) -> dict[str, Any]:
        """Trigger multiple sources in parallel.

        Args:
            sources: Restrict to these source names (default: all).
            config: Shared config passed to every source adapter.
            per_source_config: Per-source config dicts keyed by source name.
                Only the matching source receives its config, preventing
                credential leakage across adapters.
        """
        target = [s for s in self.ALL_SOURCES if s in (sources or self.ALL_SOURCES)]
        per_source_config = per_source_config or {}
        started: list[str] = []
        for source_name in target:
            runner = self._runners[source_name]
            if runner.is_running:
                continue
            source_cfg = {**(config or {}), **per_source_config.get(source_name, {})}
            result = await runner.trigger(config=source_cfg or None)
