...
def _validate_password_complexity(password: str) -> str:
    ...
    digit.  Minimum length of 8 characters is enforced at the field level;
    runtime config may enforce a higher minimum.

    Args:
        password: The plaintext password to validate.

    Returns:
        The password unchanged if all checks pass.

    Raises:
        ValueError: If the password fails any complexity check.
    """
...
class RegisterRequest(BaseModel):
    ...
    def validate_password_strength(cls, v: str) -> str:
        """Enforce password complexity on the registration password."""
...
class TokenPayload(BaseModel):
    ...
    tenant_id: str | None = None  # bound tenant for isolation enforcement
