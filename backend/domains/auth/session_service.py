...
async def find_active_session_by_family(
    db: AsyncIOMotorDatabase,  # type: ignore[type-arg]
    family_id: str,
) -> str | None:
    """Find the session ID for an active session linked to a refresh token family.

    Args:
        db: Motor database handle.
        family_id: The refresh token family ID.
