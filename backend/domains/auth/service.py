...
def create_access_token(
    ...
    *,
    ...
    Args:
        ...
        tenant_id: Tenant ID to bind this token to for isolation enforcement.
...
async def rotate_refresh_token(
    ...
    *,
    ...
    # Decode — verify issuer to reject forged tokens
    try:
        payload = jwt.decode(
            token,
            settings.jwt_secret_key,
    ...
    # Look up session_id from the refresh token's family in the sessions collection
    if not session_id and family_id:
        session_doc = await db["sessions"].find_one(
            {"refresh_token_family": family_id, "is_active": True},
            {"session_id": 1},
        )
        if session_doc:
...
async def revoke_family(
    ...
    try:
        ...
        logger.warning("Failed to decode token for family revocation: {}", exc)
        return 0
...
def _doc_to_user(doc: dict[str, Any]) -> User:
    ...
    # Decrypt TOTP secret at rest — decrypt_field handles both encrypted
    # (enc:-prefixed) and legacy plaintext values transparently.
    raw_totp = doc.get("totp_secret")
    totp_secret = decrypt_field(raw_totp) if raw_totp else None
...
async def authenticate_user(
    ...
    if not hashed or not verify_password(password, hashed):
        # Atomically increment failed attempt counter and read post-increment value
        settings = get_settings()
        ...
        new_count = updated_doc.get("failed_login_attempts", 1) if updated_doc else 1
        if new_count >= settings.account_lockout_threshold:
            locked_until = utc_now() + timedelta(minutes=settings.account_lockout_duration_minutes)
...
async def list_users(
    ...
    Returns user documents (excluding sensitive fields) and total count.
    ...
    cursor = (
        db["users"]
        .find({}, {"hashed_password": 0, "totp_secret": 0, "password_history": 0})
        .sort("username", 1)
        .skip(skip)
        .limit(limit)
    )
...
async def create_user(
    ...
    Args:
        db: Motor database handle.
        user_doc: The user document to insert.

    Returns:
        The inserted document's ID as a string.

    Raises:
        pymongo.errors.DuplicateKeyError: If username or email already exists.
    """
...
async def update_user_disabled(
    ...
    *,
    ...
    """Update a user's disabled flag and account status.

    Args:
        db: Motor database handle.
        username: Username of the user to update.
        disabled: Whether the account should be disabled.
        status: New account status value.
    """
    await db["users"].update_one(
        {"username": username},
        {"$set": {"disabled": disabled, "status": status}},
    )
...
async def update_user_status(
    ...
    *,
    ...
    """Update a user's lifecycle status and disabled flag.
    ...
    await db["users"].update_one(
        {"username": username},
        {"$set": {"status": status, "disabled": disabled}},
    )
...
async def soft_delete_user(
    ...
    """Soft-delete a user by setting status to 'deleted' and disabled to True.

    The user document is retained for audit trail integrity.

    Args:
        db: Motor database handle.
        username: Username of the user to soft-delete.
    """
    await db["users"].update_one(
        {"username": username},
        {
...
async def activate_totp(
    ...
    """Activate TOTP 2FA for a user after successful code verification.

    Args:
        db: Motor database handle.
        username: Username of the user to activate TOTP for.
    """
    await db["users"].update_one(
        {"username": username},
...
async def change_user_password(
    ...
    *,
    ...
    """Update a user's password hash and password history.

    Args:
        db: Motor database handle.
        username: Username of the user whose password is being changed.
        new_hash: The new bcrypt password hash.
        password_history: Updated password history list.
    """
    await db["users"].update_one(
        {"username": username},
        {
