...
# Login rate limiting is handled by the global RateLimitMiddleware (5 req/min
# for /api/v1/auth/login) so no per-router limiter is needed here.
...
async def update_user_disabled(
    ...
    Guards:
        - Self-modification: users cannot disable/enable themselves.
        - Privilege hierarchy: only super_admin can disable admin or
          super_admin users.
    """
    ...
    if username == current_user.sub:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot disable or enable your own account",
        )
    user = await get_user_by_username(db, username)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    # Protect admin/super_admin targets — only super_admin can modify them
    if (
        user.role in (UserRole.admin, UserRole.super_admin)
        and current_user.role != UserRole.super_admin.value
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only super_admin can disable or enable admin/super_admin users",
        )
...
async def update_user_status(
    ...
    Guards:
        - Self-modification: users cannot change their own status.
        - Privilege hierarchy: only super_admin can change the status of
    ...
    if (
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only super_admin can change admin/super_admin user status",
        )
...
async def saml_callback(
    ...
    return RedirectResponse(url=f"/auth/saml/complete?{params}", status_code=302)
