...
async def login(
    ...
    await audit(
        ...
        details={"ip_address": _get_client_ip(request), "user_agent": _get_user_agent(request)},
    )
...
async def refresh(
    ...
    # Extract session_id from the current access token if available
    ...
    if auth_header.startswith("Bearer "):
        try:
            ...
            current_payload = verify_token(auth_header[7:])
            session_id = current_payload.sid
        except ValueError:
            pass

    try:
        tokens = await rotate_refresh_token(db, payload.refresh_token, session_id=session_id)
...
async def logout(
    ...
    try:
        ...
        if family_id:
            ...
            if session_doc:
                await revoke_session(db, session_doc["session_id"], reason="logout")
    except Exception:
        pass
...
async def update_user_disabled(
    ...
    await db["users"].update_one(
        {"username": username},
        {"$set": {"disabled": payload.disabled, "status": new_status.value}},
    )
...
async def update_user_status(
    ...
    await db["users"].update_one(
        {"username": username},
        {"$set": {"status": payload.status.value, "disabled": disabled}},
    )
...
async def saml_callback(
    ...
    from utils.dt import utc_now as _utc_now
...
async def saml_token_exchange(
    ...
    doc = await db["saml_token_exchange"].find_one_and_delete({"_id": nonce})
    if doc is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
