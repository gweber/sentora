...
async def get_authorization_url(db: AsyncIOMotorDatabase) -> str:  # type: ignore[type-arg]
    ...
    await db["oidc_pending_states"].insert_one({"_id": state, "created_at": datetime.now(UTC)})
...
async def validate_state(db: AsyncIOMotorDatabase, state: str) -> bool:  # type: ignore[type-arg]
    """Validate and consume a CSRF state token.

    Atomically finds and deletes the state from MongoDB. Returns True if the
    state existed (and was therefore valid), False otherwise. Expiry is handled
    by a TTL index on the ``oidc_pending_states`` collection.
