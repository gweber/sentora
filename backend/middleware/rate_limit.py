...
# NOTE: In-memory rate limiters are per-worker. For security-critical endpoints
# (login, TOTP), the auth router uses per-username lockout stored in MongoDB
# (account_lockout_threshold) which IS shared across workers. The in-memory
# rate limit here is a supplementary defense layer.
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:  # type: ignore[override]
        ...
        # Normalize: decode URL-encoding, collapse multiple slashes, strip trailing slash
        path = re.sub(r"/+", "/", unquote(raw_path)).rstrip("/") or "/"
        ...
        strict = next(
            (
                lim
                for p, lim in self._strict_limiters.items()
