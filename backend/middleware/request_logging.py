...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        # Sanitize: accept only alphanumeric, hyphens, and underscores (max 128 chars)
        if raw_rid and len(raw_rid) <= 128 and all(c.isalnum() or c in "-_" for c in raw_rid):
            correlation_id = raw_rid
        else:
