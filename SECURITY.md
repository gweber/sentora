...
| 1.0.0 (not yet released) | Will be supported for 12 months after release |
| Older versions | Not supported |

Because Sentora has not yet reached a stable 1.0.0 release, the `main` branch is the only supported version. We encourage all users to run from the latest commit.
...
- **In-memory rate limiting.** The inbound API rate limiter (100 req/min per IP, configurable via `RATE_LIMIT_PER_MINUTE`) uses in-memory sliding windows. In a multi-worker deployment, each worker tracks limits independently. For shared rate limiting across workers, a Redis-backed implementation is recommended.
