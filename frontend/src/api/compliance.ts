...
/** Get violations export CSV URL. */
export function getViolationsExportUrl(framework?: string, severity?: string): string {
  const params = new URLSearchParams()
  if (framework) params.set('framework', framework)
  if (severity) params.set('severity', severity)
  const qs = params.toString()
  return `/api/v1/compliance/violations/export${qs ? '?' + qs : ''}`
}
