...
export function useWebSocket(url: string, onMessage: (data: unknown) => void) {
  ...
  function connect(): void {
    ...
    ws.onclose = (event) => {
      ...
      // Auth rejection — wait for token refresh then auto-reconnect
      if (event?.code === 4001) {
        lastError.value = 'Authentication failed'
        _waitForTokenRefresh()
        return
      }
      scheduleReconnect()
    }
  ...
  /**
   * After a 4001 auth rejection, poll for token refresh and auto-reconnect.
   */
  function _waitForTokenRefresh(): void {
    let polls = 0
    const pollInterval = setInterval(() => {
      polls++
      if (localStorage.getItem('sentora_token')) {
