...
onMounted(() => {
  ...
  loadDeploymentInfo()
  loadTenants()
  // Pre-fetch compliance/enforcement so the sidebar violation badge is populated
...
const nav = [
  ...
  { label: 'Compliance',         path: '/compliance',             icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
  { label: 'Enforcement',        path: '/enforcement',            icon: 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z' },
  // Fingerprinting
...
const bottomNav = computed(() => {
  ...
  // Admin-only: user management, integrations, operations
  if (auth.isAdmin) {
    // Insert admin items after Settings, before Audit Log
    items.splice(2, 0,
      ...
      { label: 'Webhooks', path: '/webhooks', icon: 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1' },
  ...
  if ((isOnprem.value && auth.isAdmin) || auth.isSuperAdmin) {
    // Insert before Getting Started (last item)
    items.splice(items.length - 1, 0, { label: 'Library Sources', path: '/library/sources', icon: 'M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4' })
  }
  // Tenants: SaaS only, super_admin only
  if (isSaas.value && auth.isSuperAdmin) {
    // Insert before Getting Started (last item)
    items.splice(items.length - 1, 0, { label: 'Tenants', path: '/tenants', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4' })
  }
  return items
})
...
<template>
  ...
  <div v-else class="flex h-screen overflow-hidden">
    ...
    <a
      href="#main-content"
      class="sr-only focus:not-sr-only focus:absolute focus:z-50 focus:top-2 focus:left-2 focus:px-4 focus:py-2 focus:bg-[var(--brand-primary)] focus:text-white focus:rounded focus:font-semibold focus:text-sm"
    ...
    >
      ...
      <nav class="flex-1 overflow-y-auto py-3 space-y-0.5" :class="collapsed ? 'px-1.5' : 'px-2'" role="navigation" aria-label="Primary">
        <router-link
          ...
          :class="[
            ...
            isActive(item.path) ? 'bg-[var(--brand-primary)]/[0.15] text-white' : 'text-slate-400 hover:bg-white/[0.045] hover:text-slate-100'
          ]"
        >
          <span
            v-if="!collapsed"
            class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-4 rounded-r-full bg-[var(--brand-primary-light)] transition-opacity duration-100"
            :class="isActive(item.path) ? 'opacity-100' : 'opacity-0'"
            aria-hidden="true"
          />
          <svg
            class="w-[15px] h-[15px] shrink-0 transition-colors duration-100"
            :class="isActive(item.path) ? 'text-[var(--brand-primary-light)]' : 'text-slate-500 group-hover:text-slate-300'"
      ...
      <div class="shrink-0 border-t border-white/[0.06]">
        ...
        >
          <!-- Avatar circle -->
          <div class="w-7 h-7 rounded-full bg-[var(--brand-primary)]/20 flex items-center justify-center shrink-0" :title="collapsed ? `${auth.user.username} (${auth.user.role})` : undefined">
            <span class="text-[11px] font-semibold text-[var(--brand-primary-light)] uppercase">{{ auth.user.username.charAt(0) }}</span>
          </div>
