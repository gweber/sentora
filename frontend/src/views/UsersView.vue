...
const roleBadge: Record<string, string> = {
  ...
  admin: 'bg-[var(--error-bg)] text-[var(--error-text)] border-[var(--error-border)]',
  analyst: 'bg-[var(--warn-bg)] text-[var(--warn-text)] border-[var(--warn-border)]',
...
<template>
  <div class="p-6 max-w-5xl mx-auto">
    ...
    >
      {{ error }}
      <button @click="error = null" class="ml-2 text-[var(--error-text)] hover:text-[var(--error-text)]">&times;</button>
    </div>
    ...
    <div class="mb-4">
      <input
        ...
        class="w-full max-w-xs px-3 py-2 text-sm rounded-lg outline-none transition focus:ring-1 focus:ring-[var(--brand-primary)]"
    ...
    <div v-if="loading" class="text-center py-12">
      <svg class="animate-spin w-6 h-6 text-[var(--brand-primary)] mx-auto mb-2" fill="none" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <tr v-for="u in filteredUsers" :key="u.username" class="transition-colors" style="border-bottom: 1px solid var(--border-light);">
            ...
            <td class="px-4 py-3 text-center">
              <span
                ...
                :class="u.totp_enabled ? 'bg-[var(--success-bg)] text-[var(--success-text)]' : 'badge-neutral-muted'"
              >
            ...
            <td class="px-4 py-3 text-right">
              <div class="flex items-center justify-end gap-1">
                <button
                  ...
                  class="p-1.5 rounded-lg hover:text-[var(--warn-text)] hover:bg-[var(--warn-bg)] transition"
                ...
                <button
                  ...
                  class="p-1.5 rounded-lg hover:text-[var(--error-text)] hover:bg-[var(--error-bg)] transition"
                  style="color: var(--text-3);"
  ...
  <div v-if="confirmDelete" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4" @click.self="confirmDelete = null">
    <div class="rounded-xl shadow-xl w-full max-w-xs p-5" style="background: var(--surface); border: 1px solid var(--border);" role="dialog" aria-modal="true" aria-label="Confirm delete user">
      <div class="text-center">
        <div class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-[var(--error-bg)] mb-3">
          <svg class="w-5 h-5 text-[var(--error-text)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
        ...
        <div class="flex gap-2">
          ...
          <button
            ...
            class="flex-1 py-2 px-3 text-sm font-medium text-white bg-[var(--error-text)] hover:bg-[var(--error-text)] rounded-lg transition"
          >
            Delete
          </button>
        </div>
