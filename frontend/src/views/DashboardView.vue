...
onMounted(() => {
  ...
  classStore.fetchOverview()
  complianceStore.fetchDashboard()
  enforcementStore.fetchSummary()
...
const osTrack: Record<string, string> = {
  windows: 'bg-blue-500/10',
  macos: 'bg-slate-500/10',
...
<template>
  <div class="p-6 max-w-[1400px] mx-auto space-y-6">
    ...
    <!-- Error banner -->
    <div v-if="loadErrors.length > 0" class="rounded-xl px-5 py-3 text-[12px] font-medium text-amber-700 bg-amber-50 border border-amber-200">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-6 gap-4" aria-live="polite">
      ...
      <div class="rounded-xl p-4" :class="(classStore.overview?.misclassified ?? 0) + (classStore.overview?.ambiguous ?? 0) > 0 ? 'bg-amber-50 border-amber-200' : ''" :style="(classStore.overview?.misclassified ?? 0) + (classStore.overview?.ambiguous ?? 0) > 0 ? '' : 'background: var(--surface); border: 1px solid var(--border);'" aria-label="Anomalies count">
        <p class="text-[10px] font-semibold uppercase tracking-widest mb-2" :class="(classStore.overview?.misclassified ?? 0) + (classStore.overview?.ambiguous ?? 0) > 0 ? 'text-amber-500' : ''" :style="(classStore.overview?.misclassified ?? 0) + (classStore.overview?.ambiguous ?? 0) > 0 ? '' : 'color: var(--text-3);'">Anomalies</p>
        <p class="text-[28px] font-bold leading-none tabular-nums" :class="(classStore.overview?.misclassified ?? 0) + (classStore.overview?.ambiguous ?? 0) > 0 ? 'text-amber-600' : ''" :style="(classStore.overview?.misclassified ?? 0) + (classStore.overview?.ambiguous ?? 0) > 0 ? '' : 'color: var(--text-1);'">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      >
        ...
        <p
          ...
          :style="{ color: complianceScore === null ? 'var(--text-3)' : complianceScore >= 90 ? 'var(--success-text)' : complianceScore >= 70 ? 'var(--warning-text)' : 'var(--error-text)' }"
        >
      ...
      <div class="rounded-xl p-5" style="background: var(--surface); border: 1px solid var(--border);">
        ...
        <div v-if="fleet" class="space-y-1.5" aria-live="polite">
          <div v-for="[label, val] in [['> 7 days', fleet.stale_7d], ['> 14 days', fleet.stale_14d], ['> 30 days', fleet.stale_30d]]" :key="label" class="flex items-center justify-between">
            ...
            <span class="text-[12px] font-semibold tabular-nums" :class="(val as number) > 0 ? 'text-rose-500' : ''" :style="(val as number) > 0 ? '' : 'color: var(--text-3);'">
    ...
    <div class="grid grid-cols-3 gap-4">
      ...
      <div class="rounded-xl p-5 space-y-5" style="background: var(--surface); border: 1px solid var(--border);">
        ...
        <div v-if="apps" aria-live="polite">
          ...
          <div class="grid grid-cols-3 gap-3">
            ...
            <div class="rounded-lg bg-amber-50 px-3 py-2.5 text-center" aria-label="Unique apps installed on only one agent">
              <p class="text-[20px] font-bold text-amber-600 tabular-nums">{{ fmtNum(apps.unique_apps) }}</p>
              <p class="text-[10px] text-amber-400 mt-0.5">Unique (1 agent)</p>
            </div>
          </div>
        </div>
      ...
      <div class="space-y-4">
        ...
        <div class="rounded-xl p-5" style="background: var(--surface); border: 1px solid var(--border);">
          ...
          <div v-if="fingerprinting" class="space-y-3" aria-live="polite">
            ...
            <div class="grid grid-cols-2 gap-2 pt-1">
              ...
              <div class="rounded-lg px-3 py-2 text-center" :class="fingerprinting.thin_fingerprints > 0 ? 'bg-amber-50' : ''" :style="fingerprinting.thin_fingerprints > 0 ? '' : 'background: var(--surface-inset);'">
                <p class="text-[18px] font-bold tabular-nums" :class="fingerprinting.thin_fingerprints > 0 ? 'text-amber-600' : ''" :style="fingerprinting.thin_fingerprints > 0 ? '' : 'color: var(--text-1);'">{{ fingerprinting.thin_fingerprints }}</p>
                <p class="text-[10px]" :class="fingerprinting.thin_fingerprints > 0 ? 'text-amber-400' : ''" :style="fingerprinting.thin_fingerprints > 0 ? '' : 'color: var(--text-3);'">Thin (&lt;3 markers)</p>
              </div>
              <div class="rounded-lg px-3 py-2 text-center" :class="fingerprinting.pending_proposals > 0 ? 'bg-indigo-50' : ''" :style="fingerprinting.pending_proposals > 0 ? '' : 'background: var(--surface-inset);'">
                <p class="text-[18px] font-bold tabular-nums" :class="fingerprinting.pending_proposals > 0 ? 'text-indigo-600' : ''" :style="fingerprinting.pending_proposals > 0 ? '' : 'color: var(--text-1);'">{{ fingerprinting.pending_proposals }}</p>
                <p class="text-[10px]" :class="fingerprinting.pending_proposals > 0 ? 'text-indigo-400' : ''" :style="fingerprinting.pending_proposals > 0 ? '' : 'color: var(--text-3);'">Pending proposals</p>
              </div>
              <div class="rounded-lg bg-rose-50 px-3 py-2 text-center" v-if="fingerprinting.groups_without_fingerprint > 0">
                <p class="text-[18px] font-bold text-rose-600 tabular-nums">{{ fingerprinting.groups_without_fingerprint }}</p>
                <p class="text-[10px] text-rose-400">No fingerprint</p>
              </div>
              <div class="rounded-lg px-3 py-2 text-center" style="background: var(--surface-inset);" v-else>
                <p class="text-[18px] font-bold text-emerald-600 tabular-nums">✓</p>
        ...
        <div class="rounded-xl p-5 flex flex-col gap-2" style="background: var(--surface); border: 1px solid var(--border);">
          ...
          <button
            class="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-[12px] font-medium transition-colors"
            @click="triggerSync"
          >
          ...
          <router-link to="/anomalies" aria-label="View Anomalies" class="flex items-center gap-2 px-3 py-2.5 rounded-lg text-[12px] font-medium transition-colors no-underline" style="border: 1px solid var(--border); color: var(--text-2);">
            <svg class="w-4 h-4 shrink-0 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
          ...
          <router-link to="/fingerprints/proposals" aria-label="Fingerprint Proposals" class="flex items-center gap-2 px-3 py-2.5 rounded-lg text-[12px] font-medium transition-colors no-underline" style="border: 1px solid var(--border); color: var(--text-2);">
            <svg class="w-4 h-4 shrink-0 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
