  ...
  <div class="p-6 space-y-5 max-w-[860px]">
    ...
    <div class="rounded-xl overflow-hidden" style="background: var(--surface); border: 1px solid var(--border);">
      ...
      <div class="px-6 py-4 flex flex-wrap items-center gap-3" style="border-bottom: 1px solid var(--border-light);">
        <button
          class="flex items-center gap-2 px-4 py-2 rounded-lg text-[13px] font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed bg-indigo-600 hover:bg-indigo-700 text-white"
          :disabled="isSyncing"
        ...
        <button
          class="flex items-center gap-2 px-4 py-2 rounded-lg text-[13px] font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed border border-emerald-200 text-emerald-700 hover:border-emerald-400 hover:text-emerald-700"
        ...
        <button
          v-if="isSyncing"
          class="flex items-center gap-2 px-4 py-2 rounded-lg text-[13px] font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed border border-red-200 text-red-600 hover:border-red-400 hover:text-red-700"
          style="background: var(--surface);"
      ...
      <div class="px-6 py-4" style="border-bottom: 1px solid var(--border-light);">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-5 gap-2">
          <button
            ...
            :class="isPhaseRunning(phase.key) ? 'border-indigo-300 bg-indigo-50' : ''"
          ...
          >
            <span class="flex items-center gap-1.5 text-[12px] font-semibold" style="color: var(--text-1);">
              <svg v-if="isPhaseRunning(phase.key)" class="w-3 h-3 animate-spin text-indigo-500 shrink-0" fill="none" viewBox="0 0 24 24">
      ...
      <div class="px-6 py-3 flex items-center gap-3 min-h-[44px]" style="background: var(--surface-inset);" aria-live="polite">
        ...
        <span
          ...
          class="ml-auto shrink-0 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-red-100 text-red-600"
        >
      ...
      <div v-if="activeCounts" class="px-6 py-5 space-y-4">
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2.5">
              <span class="w-4 h-4 flex items-center justify-center shrink-0">
                <svg v-if="phaseStatus(phase.key) === 'active'" class="w-3.5 h-3.5 animate-spin text-indigo-500" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3"/>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                </svg>
                <svg v-else-if="phaseStatus(phase.key) === 'done'" class="w-3.5 h-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            ...
            <div class="flex items-center gap-2">
              ...
              <span
                ...
                :class="{
                  'text-indigo-600': phaseStatus(phase.key) === 'active',
                }"
          ...
          <div
            ...
            :class="{
              'bg-indigo-100':  phaseStatus(phase.key) === 'active',
              'bg-emerald-100': phaseStatus(phase.key) === 'done',
          ...
          >
            <div
              ...
              :class="{
                'bg-indigo-500':  phaseStatus(phase.key) === 'active',
                'bg-emerald-500': phaseStatus(phase.key) === 'done',
    ...
    <div class="rounded-xl overflow-hidden" style="background: var(--surface); border: 1px solid var(--border);">
      ...
      <div class="px-6 py-4 flex items-center justify-between gap-4">
        ...
        <div class="shrink-0 flex items-center gap-3">
          <span
            ...
            class="text-[12px] text-emerald-600 font-medium"
          >Started in background ✓</span>
          <span
            v-else-if="backfillStatus === 'error'"
            class="text-[12px] text-red-500 font-medium"
      ...
      <div class="px-6 py-4 flex items-center justify-between gap-4" style="border-top: 1px solid var(--border-light);">
        ...
        <div class="shrink-0 flex items-center gap-3">
          <span v-if="renormalizeStatus === 'started'" class="text-[12px] text-emerald-600 font-medium">Started in background ✓</span>
          <span v-else-if="renormalizeStatus === 'error'" class="text-[12px] text-red-500 font-medium">Failed — check server logs</span>
          <button
            class="flex items-center gap-2 px-4 py-2 rounded-lg text-[13px] font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            ...
            >
              ...
              <td class="px-4 py-3">
                <span
                  ...
                  :class="run.mode === 'full' ? 'bg-indigo-50 text-indigo-600' : ' text-muted'" style="background: var(--surface-hover);"
                >{{ run.mode }}</span>
              </td>
              <td class="px-4 py-3">
                <span
              ...
              <td
                ...
                :class="run.counts.errors > 0 ? 'text-red-500 font-semibold' : ''"
        ...
        <template v-else>
          ...
          <div class="flex items-center justify-between mt-4">
            <div class="flex items-center gap-3 ml-auto">
              <span v-if="limitsStatus === 'saved'" class="text-[12px] font-medium text-emerald-600 flex items-center gap-1">
                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                Saved
              </span>
              <span v-else-if="limitsStatus === 'error'" class="text-[12px] font-medium text-red-500">Save failed</span>
              <button
