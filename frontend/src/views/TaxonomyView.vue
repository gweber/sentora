    ...
    <main class="flex-1 overflow-y-auto p-6">
      ...
      <template v-else>
        ...
        <div class="flex items-center justify-between mb-5">
          ...
          <button
            class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600 text-white text-[13px] font-medium hover:bg-indigo-700 transition-colors"
            @click="openAdd"
          >
        ...
        <div v-if="deleteError" class="text-[12px] font-medium text-red-500 mb-3 px-1">{{ deleteError }}</div>
            ...
            <div class="flex items-start justify-between gap-3">
              ...
              <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2 mb-1.5 flex-wrap">
                  ...
                  <span
                    ...
                    class="text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600 border border-indigo-100"
                  >
                    Universal
                  </span>
                  <span
                    v-if="entry.user_added"
                    class="text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded bg-amber-50 text-amber-600 border border-amber-100"
                  >
                    Custom
                  </span>
                </div>
                ...
                <div
                  ...
                  class="mt-2 rounded-lg border border-indigo-200 bg-indigo-50/50 p-3"
                >
                  <!-- Loading -->
                  <div v-if="previewLoading" class="text-[11px] text-indigo-500 flex items-center gap-1.5">
                  ...
                  <template v-else>
                    ...
                    <div class="flex items-center gap-3 mb-2.5">
                      <span class="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-indigo-200 text-indigo-700">
                        {{ previewData.total_apps }} app{{ previewData.total_apps !== 1 ? 's' : '' }}
                      </span>
                      <span class="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
                    ...
                    <div class="overflow-y-auto max-h-[180px] mb-2">
                      <table class="w-full text-[11px]">
                        <thead class="bg-indigo-100/50 sticky top-0">
                          <tr>
                            <th scope="col" class="text-left px-2 py-1 font-semibold text-indigo-600">App Name</th>
                            <th scope="col" class="text-left px-2 py-1 font-semibold text-indigo-600">Publisher</th>
                            <th scope="col" class="text-right px-2 py-1 font-semibold text-indigo-600">Agents</th>
                          </tr>
                        </thead>
                        <tbody class="divide-y divide-indigo-100">
                          <tr v-for="m in previewData.app_matches" :key="m.normalized_name" class="hover:bg-indigo-50/50">
                    ...
                    <div v-if="previewData.group_counts.length > 0">
                      <p class="text-[10px] font-semibold text-indigo-600 uppercase tracking-wide mb-1">By Group</p>
                      <div class="flex flex-wrap gap-1.5">
                        <span
                          v-for="g in previewData.group_counts"
                          :key="g.group_name"
                          class="text-[10px] px-1.5 py-0.5 rounded border border-indigo-200" style="background: var(--surface); color: var(--text-2);"
                        >
                          {{ g.group_name }} <span class="font-semibold text-indigo-600">{{ g.agent_count }}</span>
                        </span>
                      </div>
                    </div>
                  </template>
              ...
              <div class="flex items-center gap-1 shrink-0 pt-0.5">
                ...
                <template v-if="deleteConfirmId === entry.id">
                  <span class="text-[11px] text-red-600 font-medium">Delete?</span>
                  <button
                    :disabled="deleting"
                    class="px-2 py-1 rounded text-[11px] font-medium bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 transition-colors"
                ...
                <button
                  v-else
                  title="Delete entry"
                  class="p-1.5 rounded-md hover:bg-red-50 hover:text-red-500 transition-colors"
                  style="color: var(--text-3);"
          ...
          <form class="px-6 py-5 space-y-4" @submit.prevent="save">
            ...
            <div>
              ...
              <textarea
                ...
                class="w-full rounded-lg px-3 py-2 text-[13px] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none"
                style="border: 1px solid var(--input-border); background: var(--surface); color: var(--text-1);"
              />
            </div>
            ...
            <p v-if="saveError" class="text-[12px] text-red-600">{{ saveError }}</p>
