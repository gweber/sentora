...
import type { UnifiedViolation, UnifiedViolationListResponse } from '@/api/compliance'
...
function statusBadgeStyle(status: string): { text: string; style: string } {
  switch (status) {
    ...
    case 'warning': return { text: 'Warn', style: 'background: var(--warning-bg); color: var(--warning-text);' }
...
function downloadViolations() {
  const url = complianceApi.getViolationsExportUrl(filterFramework.value, filterSeverity.value)
      ...
      <div v-if="store.dashboard" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        >
          ...
          <div class="flex gap-3 mt-2 text-xs" style="color: var(--text-3);">
            <span class="text-green-600">{{ fw.passed }} pass</span>
            <span class="text-red-600">{{ fw.failed }} fail</span>
            <span class="text-yellow-600">{{ fw.warning }} warn</span>
          </div>
        </div>
      </div>
