...
async function downloadViolations() {
  try {
    ...
    toast.show(err instanceof Error ? err.message : 'Failed to download violations', 'error')
  }
}
...
async function loadUnifiedViolations(page: number = 1) {
  ...
  try {
    ...
    toast.show(err instanceof Error ? err.message : 'Failed to load violations', 'error')
  }
}
