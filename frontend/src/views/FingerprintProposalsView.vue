...
<template>
  <div class="p-6 max-w-[960px] space-y-5">
    ...
    <transition name="fade">
      ...
      >
        <svg v-if="toast.type === 'success'" class="w-4 h-4 text-[var(--success-text)] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
          <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
        </svg>
        <svg v-else class="w-4 h-4 text-[var(--error-text)] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
    ...
    <div class="flex items-start justify-between gap-4">
      ...
      <div class="flex items-center gap-3 shrink-0">
        ...
        <button
          ...
          :class="isGenerating
            ? 'bg-[var(--brand-primary-light)] text-[var(--brand-primary)] cursor-not-allowed'
            : 'bg-[var(--brand-primary)] text-white hover:bg-[var(--brand-primary-dark)] shadow-sm'"
        >
          <svg
            class="w-3.5 h-3.5"
    ...
    <div v-if="isGenerating" class="bg-[var(--brand-primary-light)] border border-[var(--brand-primary-light)] rounded-xl px-5 py-4">
      <div class="flex items-center gap-3 mb-2.5">
        <svg class="w-4 h-4 text-[var(--brand-primary)] animate-spin shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
          <path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
        </svg>
        <span class="text-[13px] font-medium text-[var(--brand-primary)]">
          Analyzing installed applications across all groups…
        </span>
      </div>
      <div class="h-1.5 rounded-full bg-[var(--brand-primary-light)] overflow-hidden">
        <div class="h-full bg-[var(--brand-primary)] rounded-full animate-pulse" style="width: 60%;" />
      </div>
    </div>
    ...
    <template v-else-if="!isLoading">
      ...
      <div class="grid grid-cols-4 gap-3">
        ...
        >
          <p
            ...
            :class="totalConflicts > 0 ? 'text-[var(--warn-text)]' : ''"
      ...
      <div class="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div class="flex items-center gap-2 mr-2">
          <button
            ...
            class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)] focus:ring-offset-1"
            :class="hideSmallGroups ? 'bg-[var(--brand-primary)]' : 'bg-[var(--surface-hover)]'"
          >
            <span
          ...
          <span class="text-[12px] whitespace-nowrap" style="color: var(--text-2);">
            ...
            <input
              ...
              min="1"
              max="999"
              class="inline-block w-12 mx-1 text-center text-[12px] px-1 py-0.5 rounded focus:outline-none focus:ring-1 focus:ring-[var(--brand-primary)]"
        ...
        <div class="flex items-center gap-2">
          ...
          <select
            v-model="sortKey"
            class="text-[12px] px-2.5 py-1.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)]"
          ...
          <button
            ...
            class="px-3 py-1.5 rounded-lg text-[12px] font-semibold bg-[var(--success-text)] text-white hover:bg-[var(--success-text)] transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      <div class="space-y-3" aria-live="polite">
        <div
          ...
          :class="{
            'border-[var(--success-border)]': proposal.status === 'applied',
          }"
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-3 mt-0.5 flex-wrap">
                ...
                <span class="text-[12px] font-medium text-[var(--brand-primary)]">
                  avg {{ formatLift(proposal.quality_score) }} lift
                </span>
                <!-- Conflict badge -->
                <span
                  v-if="conflictCount(proposal) > 0"
                  class="flex items-center gap-1 text-[11px] font-medium text-[var(--warn-text)]"
                >
            ...
            <div class="flex items-center gap-2 shrink-0" @click.stop>
              <button
                ...
                :class="applyingGroups.has(proposal.group_id)
                  ...
                  : 'bg-[var(--success-text)] text-white hover:bg-[var(--success-text)]'"
              >
                {{ applyingGroups.has(proposal.group_id) ? 'Applying…' : 'Apply' }}
              </button>
              <span v-else class="text-[12px] font-semibold text-[var(--success-text)] flex items-center gap-1">
                    ...
                    <tbody class="divide-y divide-gray-50">
                      ...
                      >
                          ...
                          <td class="px-5 py-2.5 font-medium truncate max-w-[240px]" style="color: var(--heading);">
                            <button
                              class="hover:text-[var(--brand-primary)] hover:underline transition-colors text-left truncate"
                          ...
                          <td class="px-4 py-2.5">
                            <div class="flex items-center gap-2">
                              <div class="w-20 h-1.5 rounded-full  overflow-hidden shrink-0" style="background: var(--surface-hover);">
                                <div
                                  class="h-full rounded-full bg-[var(--success-text)]"
                          ...
                          <td class="px-4 py-2.5">
                            <div class="flex items-center justify-end gap-2">
                              ...
                              >
                                <span class="flex items-center gap-0.5 text-[11px] font-medium text-[var(--warn-text)] cursor-default">
                                  <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                                  </svg>
                                </span>
                              ...
                              <button
                                ...
                                :class="addedMarkers.has(`${proposal.group_id}:${marker.normalized_name}`)
                                  ? 'bg-[var(--success-bg)] text-[var(--success-text)] cursor-default'
