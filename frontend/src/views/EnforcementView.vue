  ...
  <div class="max-w-7xl mx-auto px-4 py-6">
    ...
    <div v-if="store.summary" class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
      ...
      <div class="rounded-xl p-4" style="background: var(--surface); border: 1px solid var(--border);">
        ...
        <div class="text-2xl font-bold mt-1 text-[var(--success-text)]">{{ store.summary.passing }}</div>
      </div>
      <div class="rounded-xl p-4" style="background: var(--surface); border: 1px solid var(--border);">
        <div class="text-xs" style="color: var(--text-3);">Failing</div>
        <div class="text-2xl font-bold mt-1 text-[var(--error-text)]">{{ store.summary.failing }}</div>
      </div>
      <div class="rounded-xl p-4" style="background: var(--surface); border: 1px solid var(--border);">
            ...
            <tr v-for="rule in store.rules" :key="rule.id">
              ...
              <td class="px-4 py-3 text-right space-x-2">
                <label class="relative inline-flex items-center cursor-pointer">
                  ...
                  <div class="w-8 h-4 bg-[var(--surface-hover)] peer-checked:bg-[var(--brand-primary)] rounded-full transition-colors after:content-[''] after:absolute after:top-[1px] after:left-[1px] after:bg-white after:rounded-full after:h-3.5 after:w-3.5 after:transition-all peer-checked:after:translate-x-3.5"></div>
                </label>
                <button class="text-[var(--error-text)] hover:text-[var(--error-text)] text-xs" @click="confirmDelete(rule.id)">Delete</button>
              </td>
            </tr>
        ...
        <div class="rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" style="background: var(--surface);">
          ...
          <form class="space-y-3" @submit.prevent="handleCreate">
            ...
            <div class="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" class="px-4 py-2 bg-[var(--brand-primary)] text-white rounded-lg text-sm hover:bg-[var(--brand-primary-dark)]">Create Rule</button>
            </div>
          </form>
        </div>
      </div>
