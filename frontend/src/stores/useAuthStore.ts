...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function login(username: string, password: string): Promise<'ok' | 'totp' | 'error'> {
    ...
    try {
      ...
      if (
        ...
        // AUDIT-032: Password held in reactive state until TOTP verification completes; cleared in loginWithTotp finally block
  ...
  async function loginWithTotp(code: string): Promise<boolean> {
    ...
    try {
      ...
      setTokens(resp.access_token, resp.refresh_token)
      await fetchUser()
      // Clear credentials only on success so the user can retry on failure
  ...
  async function register(username: string, email: string, password: string): Promise<boolean> {
    ...
    try {
      ...
      // AUDIT-032: Password held in reactive state until TOTP setup verification; cleared in verifyTotpSetup finally block
  ...
  async function verifyTotpSetup(code: string): Promise<boolean> {
    ...
    try {
      ...
      // AUDIT-032: Clear plaintext password from reactive state immediately after use
