...
const mockCategory: CategorySummary = { key: 'browser', display: 'Browsers', entry_count: 3 }
const mockEntry: SoftwareEntry = {
  id: 'e1',
  name: 'Chrome',
  category: 'browser',
  category_display: 'Browsers',
  patterns: ['chrome*'],
  publisher: null,
  subcategory: null,
  industry: [],
...
describe('useTaxonomyStore', () => {
  ...
  describe('fetchCategories', () => {
    it('populates categories from API', async () => {
      vi.mocked(taxApi.listCategories).mockResolvedValue({ categories: [mockCategory], total: 1 })
  ...
  describe('fetchEntriesByCategory', () => {
    it('fetches and caches entries', async () => {
      vi.mocked(taxApi.getEntriesByCategory).mockResolvedValue({ entries: [mockEntry], total: 1 })
  ...
  describe('search', () => {
    it('populates searchResults', async () => {
      vi.mocked(taxApi.searchTaxonomy).mockResolvedValue({ entries: [mockEntry], total: 1 })
  ...
  describe('createCategory', () => {
    it('creates category and refreshes list', async () => {
      ...
      const store = useTaxonomyStore()
      const result = await store.createCategory({ key: 'browser', display: 'Browsers' })
  ...
  describe('deleteEntry', () => {
    it('deletes entry, invalidates cache, refreshes categories', async () => {
      ...
      vi.mocked(taxApi.listCategories).mockResolvedValue({ categories: [], total: 0 })
  ...
  describe('updateCategory', () => {
    it('invalidates both old and new key caches on rename', async () => {
      ...
      const result = await store.updateCategory('browser', { key: 'web_browser', display: 'Web Browsers' })
    ...
    it('only invalidates old key when key unchanged', async () => {
      ...
      await store.updateCategory('browser', { display: 'Browsers 2' })
    ...
    it('returns false on failure', async () => {
      ...
      const store = useTaxonomyStore()
      const result = await store.updateCategory('browser', { display: 'x' })
