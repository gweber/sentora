...
describe('useTagStore', () => {
  ...
  describe('updateRule', () => {
    it('updates activeRule and rules list entry', async () => {
      ...
      await store.updateRule('r1', { name: 'Updated Rule' })

      expect(store.activeRule!.name).toBe('Updated Rule')
      expect(store.rules[0].name).toBe('Updated Rule')
    })
    ...
    it('sets error on failure', async () => {
      ...
      const store = useTagStore()
      await store.updateRule('r1', { name: '' })
  ...
  describe('addPattern', () => {
    it('appends pattern to activeRule when matching', async () => {
      const newPattern: TagRulePattern = { id: 'p2', field: 'name', match_type: 'glob', value: 'firefox*' }
      vi.mocked(tagsApi.addPattern).mockResolvedValue(newPattern)

      const store = useTagStore()
      store.activeRule = { ...mockRule, patterns: [mockPattern] }

      const result = await store.addPattern('r1', { field: 'name', match_type: 'glob', value: 'firefox*' } as any)
  ...
  describe('applyRule', () => {
    it('applies rule and refreshes activeRule', async () => {
      ...
      vi.mocked(tagsApi.getRule).mockResolvedValue({ ...mockRule, name: 'Refreshed' })
      ...
      expect(store.activeRule!.name).toBe('Refreshed')
      expect(store.isApplying).toBe(false)
    })
