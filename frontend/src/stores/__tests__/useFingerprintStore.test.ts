...
describe('useFingerprintStore', () => {
  ...
  describe('addMarker', () => {
    it('adds marker to activeFingerprint markers array', async () => {
      const newMarker: FingerprintMarker = { id: 'm3', pattern: 'slack*', display_name: 'Slack', category: 'communication', weight: 0.8, source: 'manual', confidence: 1.0, added_at: '2025-01-01T00:00:00Z', added_by: 'admin' }
      ...
      const result = await store.addMarker({ pattern: 'slack*', display_name: 'Slack', category: 'communication', weight: 0.8 })
    ...
    it('returns null when no activeFingerprint', async () => {
      const store = useFingerprintStore()
      const result = await store.addMarker({ pattern: 'x', display_name: 'X', category: 'other', weight: 1 })
