...
const mockRun: SyncRun = {
  ...
  counts: { sites: 2, groups: 5, agents: 10, apps: 0 },
  started_at: '2025-01-01T00:00:00Z',
  completed_at: null,
  phases: ['sites', 'groups', 'agents', 'apps'],
}
...
describe('useSyncStore', () => {
  ...
  describe('fetchStatus', () => {
    it('populates currentRun and lastCompletedRun from API', async () => {
      ...
      vi.mocked(syncApi.getSyncStatus).mockResolvedValue({
        ...
        db_counts: { sites: 1, groups: 2, agents: 10, apps: 100 },
      })
      ...
      expect(store.dbCounts).toEqual({ sites: 1, groups: 2, agents: 10, apps: 100 })
      expect(store.isLoading).toBe(false)
      expect(store.error).toBeNull()
    })
  ...
  describe('triggerSync', () => {
    it('triggers sync and returns true on success', async () => {
      vi.mocked(syncApi.triggerSync).mockResolvedValue(mockRun)
  ...
  describe('handleProgressMessage', () => {
    ...
    it('updates currentRun in place when run_id matches', () => {
      ...
      expect(store.currentRun!.counts.apps).toBe(50)
      // Previous counts preserved
      expect(store.currentRun!.counts.agents).toBe(10)
    })
