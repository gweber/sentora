...
- **Tenant impersonation:** An authenticated user in tenant A could attempt to access tenant B's data by manipulating the `X-Tenant-ID` header. Mitigated by `TenantMiddleware` which validates tenant membership. Additionally, all repository queries inject `tenant_id` from the JWT claim, providing defence in depth at the data layer (ADR-0024).
- **Privilege escalation across tenants:** A tenant `admin` could attempt platform operations. Mitigated by `require_platform_role()` which requires `super_admin` in SaaS mode. Self-role-elevation and assignment above caller's own role are explicitly blocked (ADR-0024).
- **DNS rebinding in webhook delivery:** An attacker controlling a DNS server could cause webhook SSRF validation to pass on a public IP, then rebind to an internal IP at connection time. Mitigated by pinning the resolved IP for the duration of the HTTP request (ADR-0024).
...
- MongoDB authentication is enabled by default in Docker Compose (ADR-0023).
- TOTP secrets and backup recovery codes are encrypted/hashed at rest, limiting the impact of a database compromise (ADR-0024).

**Residual risk:** Low. With MongoDB authentication enabled and sensitive fields encrypted at rest, a database compromise yields limited usable credential material.
...
- Role assignment endpoints block self-elevation and assignment above the caller's own role (ADR-0024).
- Platform-level operations in SaaS mode strictly require `super_admin` JWT claim — header-based overrides are rejected (ADR-0024).
