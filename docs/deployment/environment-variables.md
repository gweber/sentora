...
| `FIELD_ENCRYPTION_KEY` | No | _(falls back to `JWT_SECRET_KEY`)_ | Separate key for field-level encryption (webhook secrets, API keys, etc.). Generate with: `python -c "import secrets; print(secrets.token_urlsafe(64))"`. Set a dedicated key for better security isolation from JWT signing. |
